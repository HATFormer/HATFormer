...
class pl_trainer_base(pl.LightningModule):
    ...
    def norm_train_dsm(self, x):
        if 'self.max_scale_dsm' not in locals():
            self.max_scale_dsm= 99
            self.min_scale_dsm= -10
        return 2 * (x - self.min_scale_dsm) / (self.max_scale_dsm - self.min_scale_dsm) - 1

    def norm_infer(self, x):
        return (x + 1) * (self.max_scale - self.min_scale) / 2 + self.min_scale

    def norm_infer_dsm(self, x):
        if 'self.max_scale_dsm' not in locals():
            self.max_scale_dsm= 99
            self.min_scale_dsm= -10
        return (x + 1) * (self.max_scale_dsm - self.min_scale_dsm) / 2 + self.min_scale_dsm
    ...
    def save_img_func(self, img, img_path, save_dir=None, size=(512, 512), dim=1):
        # if not save_dir:
    ...
    def save_img_func_3d_backup(self, img, img_path, save_dir=None, size=(512, 512), dsm_mask=False):
        # if not save_dir:
        save_dir = self.logger.log_dir.rstrip(self.logger.log_dir.split('/')[-1]) + save_dir
        os.makedirs(save_dir, exist_ok=True)
        # img = cv2.resize(img,size)
        ...
        if dsm_mask:
            color_img = self.vis_3d(img,self.custom_lut,
                                    (img.min(),#min(img.min(),self.mask3d.min()),
                                     img.max()#max(img.max(),self.mask3d.max()))
                                     ))
        else:
            color_img = self.vis_3d(img,self.custom_lut,
                                    (self.mask3d.min(),#min(img.min(),self.mask3d.min()),
                                     self.mask3d.max()#max(img.max(),self.mask3d.max()))
                                     ))
    ...
    def save_img_func_3d(self, img, img_path, save_dir=None, size=(512, 512)):
        # if not save_dir:
        save_dir = self.logger.log_dir.rstrip(self.logger.log_dir.split('/')[-1]) + save_dir
        os.makedirs(save_dir, exist_ok=True)
        # img = cv2.resize(img,size)
        ...
        #     dst.write(img.reshape(1,512,512))
        #img_gray = np.uint8(255 * self.norm(img, img.max(), img.min()))
        color_img = self.vis_3d(img,self.custom_lut, (img.min(), img.max()))
        color_img = cv2.cvtColor(color_img, cv2.COLOR_RGB2BGR)
